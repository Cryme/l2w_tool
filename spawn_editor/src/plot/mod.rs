...
impl Plot {
    ...
    pub fn new(id_source: impl std::hash::Hash, search_zone: Rc<RwLock<Option<Rect>>>) -> Self {
        Self {
            ...
            clamp_grid: false,
            coord_label_text_color: None,
            coords_to_square_fn: Box::new(|v| (v.x as u8, v.y as u8)),
    ...
    /// Color of text with current coords
    #[inline]
    pub fn label_text_color(mut self, color: Option<Color32>) -> Self {
        self.coord_label_text_color = color;
        self
    }

    /// Color of text with current coords
    #[inline]
    pub fn coords_to_square_fn(mut self, func: Box<CoordsToSquareFn>) -> Self {
        self.coords_to_square_fn = func;
        self
    }
    ...
    pub fn x_axis_formatter(
        mut self,
        fmt: impl Fn(GridMark, usize, &RangeInclusive<f64>, bool) -> String + 'static,
    ) -> Self {
        if let Some(main) = self.x_axes.first_mut() {
    ...
    fn show_dyn<'a, R>(
        ...
        .unwrap_or_else(|| PlotMemory {
            ...
            last_click_pos_for_spawn_search_zone: None,
        ...
        items
            .write()
            .unwrap()
            .sort_by_key(|item| item.highlighted());
        ...
        let shift_pressed = ui.ctx().input(|i| i.modifiers.shift) && !alt_pressed;
        ...
        if response.double_clicked() {
            if alt_pressed {
                ...
                let p = mem
                    .transform
                    .value_from_position(response.hover_pos().unwrap());
                *search_zone.write().unwrap() = Some(Rect {
                    min: Pos2::new(p.x as f32, -p.y as f32),
                    max: Pos2::new(p.x as f32, -p.y as f32),
                });
            }
        }
        ...
        let is_drugging =
            response.dragged_by(PointerButton::Primary) && !alt_pressed && !shift_pressed;
        // Dragging
        if allow_drag.any() && is_drugging {
        ...
        // Zone filer
        {
            if shift_pressed && mem.last_click_pos_for_zoom.is_none() {
                // Save last click to allow boxed zooming
                if response.drag_started()
                    && response.dragged_by(boxed_zoom_pointer_button)
                    && shift_pressed
                {
                    // it would be best for egui that input has a memory of the last click pos because it's a common pattern
                    mem.last_click_pos_for_spawn_search_zone = response.hover_pos();
                }

                let box_start_pos = mem.last_click_pos_for_spawn_search_zone;
                ...
                if let (Some(box_start_pos), Some(box_end_pos)) = (box_start_pos, box_end_pos) {
                    ...
                    if response.drag_stopped() {
                        ...
                        let filter_zone = Rect {
                            min: (
                                box_start_pos.x.min(box_end_pos.x) as f32,
                                (-box_start_pos.y).min(-box_end_pos.y) as f32,
                            )
                                .into(),
                            max: (
                                box_start_pos.x.max(box_end_pos.x) as f32,
                                (-box_start_pos.y).max(-box_end_pos.y) as f32,
                            )
                                .into(),
                        };

                        {
                            *search_zone.write().unwrap() = Some(filter_zone);
                        }

                        // reset the boxed zoom state
                        mem.last_click_pos_for_spawn_search_zone = None;
                    }
                }
            }
        }
        ...
        // Zooming
        {
            if mem.last_click_pos_for_spawn_search_zone.is_none() {
                // Save last click to allow boxed zooming
                if response.drag_started()
                    && response.dragged_by(boxed_zoom_pointer_button)
                    && alt_pressed
        ...
        for item in &mut *items.write().unwrap() {
            item.initialize(mem.transform.bounds().range_x());
        }

        let new_polygon = if is_in_create_mode {
            let mut pol = drawing_polygon.write().unwrap();

            let len = pol.len();

            let resp = response.hover_pos();

            if !response.ctx.input(|v| v.pointer.any_down())
                && !alt_pressed
                && !shift_pressed
                && resp.is_some()
            {
                let v = mem.transform.value_from_position(resp.unwrap());

                pol[len - 1] = [v.x, v.y];

                if ui
                    .ctx()
                    .input_mut(|i| i.consume_key(Modifiers::NONE, Key::A))
                {
                    pol.push([v.x, v.y]);
                } else if len > 1
                    && ui
                        .ctx()
                        .input_mut(|i| i.consume_key(Modifiers::NONE, Key::R))
                {
                    pol.remove(len - 1);
                }
            } else if len > 1 {
            ...
            let mut r = Polygon::new(PlotPoints::new(pol.clone()))
                .stroke((1.0, Color32::YELLOW))
                .style(LineStyle::Dotted { spacing: 5.0 })
                .fill_color(Color32::TRANSPARENT);

            r.initialize(mem.transform.bounds().range_x());
                ...
                legend_ui
                    .scope(|ui| {
                        ...
                        .show(ui, |ui| {
                            ...
                            let mut resp =
                                ui.label(RichText::new("L Mouse: Drag").color(Color32::WHITE));

                            resp = resp.union(
                                    ui.label(RichText::new("Alt+L mouse: zoom into area").color(Color32::WHITE))
                                );
                            resp = resp.union(
                                    ui.label(RichText::new("Alt+Double L mouse: reset zoom").color(Color32::WHITE))
                                );
                            resp = resp.union(
                                    ui.label(RichText::new("Shift+L Mouse: show area spawns").color(Color32::WHITE))
                                );

                            resp = resp.union(
                                    ui.label(RichText::new("Shift+Double L Mouse: show square spawns").color(Color32::WHITE))
                                );
                            ...
                            resp = resp.union(
                                ui.label(
                                    RichText::new("🎲 - random territory")
                                        .color(Color32::WHITE)
                                )
                                .on_hover_text("On server load spawn will\nselect on of random territory.\nSo it won't change until\nrestart."));
                            resp = resp.union(
                                ui.label(
                                    RichText::new("🚫 - banned territory")
                                        .color(Color32::WHITE)
                                )
                                .on_hover_text("Excluded from spawn zone"));

                            if is_in_create_mode {
                                ui.label("----- Create -----");
                                resp = resp.union(
                                    ui.label(RichText::new("A: add polygon point").color(Color32::WHITE))
                                );
                                resp = resp.union(
                                    ui.label(RichText::new("R: remove polygon point").color(Color32::WHITE))
                                );
                            }
        ...
        let response = response.on_hover_cursor(if is_drugging {
            CursorIcon::Grabbing
        } else {
...
impl PreparedPlot {
    fn ui(self, ui: &mut Ui, response: &Response) -> (Vec<Cursor>, Option<Id>) {
        ...
        if let Some(poly) = &self.new_polygon {
            poly.shapes(&plot_ui, transform, &mut shapes);
        }
    ...
    fn paint_grid(&self, ui: &Ui, shapes: &mut Vec<(Shape, f32)>, axis: Axis, fade_range: Rangef) {
        ...
        let clamp_range = clamp_grid.then(|| {
            ...
            if let Some(p) = &self.new_polygon {
                let item_bounds = p.bounds();
                tight_bounds.merge_x(&item_bounds);
                tight_bounds.merge_y(&item_bounds);
            }
            tight_bounds
    ...
    fn hover(&self, ui: &Ui, pointer: Pos2, shapes: &mut Vec<Shape>) -> (Vec<Cursor>, Option<Id>) {
        ...
        let plot = items::PlotConfig {
            ui,
            transform,
            coord_label_text_color: *coord_label_text_color,
            coords_to_square_fn,
        };

        let mut cursors = Vec::new();

        let hovered_plot_item_id = if let Some((item, elem)) = items
            .read()
            .unwrap()
            .iter()
        ...
        {
            item.on_hover(elem, shapes, &mut cursors, &plot);
            item.id()
        } else {
            let value = transform.value_from_position(pointer);
            items::rulers_at_value(pointer, value, "", &plot, shapes, &mut cursors, false);
            None
        };
