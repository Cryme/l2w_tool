...
impl From<[f64; 4]> for PlotPoint {
    #[inline]
    fn from([x, y, z_min, z_max]: [f64; 4]) -> Self {
        Self {
            x,
            y,
            z_min: z_min as i32,
...
impl From<Vec<[f64; 4]>> for PlotPoints {
    fn from(coordinates: Vec<[f64; 4]>) -> Self {
        Self::new_with_z(coordinates)
    }
}
...
impl FromIterator<[f64; 4]> for PlotPoints {
    fn from_iter<T: IntoIterator<Item = [f64; 4]>>(iter: T) -> Self {
        Self::Owned(iter.into_iter().map(|point| point.into()).collect())
    }
}
impl FromIterator<(f64, f64)> for PlotPoints {
...
impl PlotPoints {
    ...
    pub fn new_with_z(points: Vec<[f64; 4]>) -> Self {
        Self::from_iter(points)
    }
